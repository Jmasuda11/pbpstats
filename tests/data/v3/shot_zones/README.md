...
The WNBA captures retain conflicting corner/arc and mid-range labels (39 of 120 three-point attempts); one also has a one-unit xLegacy discrepancy. The G League capture retains zero overtime clocks where native V3 has its elapsed-time counter (two affected three-pointers); explicit target-score metadata, period boundaries and the complete scoring sequence corroborate those joins, validating all 61 attempts without changing either source. The NBA game's 69 three-point attempts all agree. The production adapter preserves unresolved per-shot failures and raises on dependent access, while possession/time accounting remains available.
